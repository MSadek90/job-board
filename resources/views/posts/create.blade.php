<x-layout :title="$page_title ?? 'Create Post'">
    <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-10 px-4">
        <div class="max-w-2xl mx-auto">
            ...
            {{-- Page Header --}}
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-slate-800 tracking-tight">Create New Post</h1>
                <p class="mt-2 text-slate-500 text-sm">Fill in the details below to publish a new post.</p>
            </div>
                ...
                <form action="/posts" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <input id="title" type="text" name="title" value="{{ old('title') }}" required
                            placeholder="{{ $title_placeholder ?? 'Post Title' }}"
                    ...
                    <div class="mb-6">
                        ...
                        <input id="author" type="text" name="author" value="{{ old('author') }}" required
                            placeholder="{{ $author_placeholder ?? 'Author Name' }}"
                    ...
                    <div class="mb-8">
                        ...
                        <textarea id="description" name="description" rows="5" required
                            placeholder="{{ $description_placeholder ?? 'Write your post description...' }}"
                            class="block w-full bg-slate-50 border {{ $errors->has('description') ? 'border-red-400' : 'border-slate-200' }} text-slate-800 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 rounded-xl py-3 px-4 text-sm outline-none transition-all duration-200 resize-none">{{ old('description') }}</textarea>
                        @error('description')
                            <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-3">
                        ...
                        <button type="submit" class="rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 px-6 py-2.5 text-sm font-semibold text-white hover:from-indigo-600 hover:to-purple-700 shadow-lg shadow-indigo-200 transition-all duration-200">Save Post</button>
                    </div>
                </form>
            </div>
