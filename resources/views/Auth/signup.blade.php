<x-auth :title="$page_title?? signup">
    <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 px-4">
        <div class="w-full max-w-md bg-white rounded-2xl shadow-md border border-slate-200 p-6">
            ...
            <form action="{{ route('signup.show') }}" method="POST">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}"
                        class="w-full px-4 py-2 border rounded-xl focus:ring-2 focus:ring-indigo-500 @error('name') border-red-400 @else border-slate-300 @enderror"
                        required>
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <input type="email" name="email" value="{{ old('email') }}"
                        class="w-full px-4 py-2 border rounded-xl focus:ring-2 focus:ring-indigo-500 @error('email') border-red-400 @else border-slate-300 @enderror"
                        required>
                    @error('email')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <input type="password" name="password_confirmation"
                        class="w-full px-4 py-2 border rounded-xl focus:ring-2 focus:ring-indigo-500 border-slate-300"
                        required>
                </div>
            ...
            <p class="text-sm text-center text-slate-500 mt-4">
                ...
                <a href="{{ route('login.show') }}" class="text-indigo-600 hover:underline">Login</a>
            </p>
